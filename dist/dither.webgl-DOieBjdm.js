...
function W(t, n, e, g, i = 1 / 0) {
  ...
  function F() {
    const o = Math.min(A, 2, Math.sqrt(24e5 / (c * a)), i / c, i / a), r = Math.max(1, Math.floor(c * o)), I = Math.max(1, Math.floor(a * o));
    return (e.width !== r || e.height !== I) && (e.width = r, e.height = I), r / c;
  }
  function v(o) {
    if (!l && (d = o, !!h()))
      try {
        ...
        t.onError ? t.onError(I) : console.error(I);
      }
  }
  function b() {
    !E && h() && (E = requestAnimationFrame(D));
  }
  function _() {
    h() ? b() : (cancelAnimationFrame(E), E = 0, m = null);
  }
  function D(o) {
    if (E = 0, !h()) {
      m = null;
    ...
    m = o, R && (L.matches || (f += r), u += (s - u) * (1 - Math.exp(-r * 7)), Math.abs(s - u) < 2e-3 && (u = s)), v(R ? f : d), R && (!L.matches || u !== s) ? b() : m = null;
  }
  function x() {
    if (l) return;
    const o = window.devicePixelRatio || 1;
    A !== o && (A = o, T.removeEventListener("change", x), T = window.matchMedia(`(resolution: ${o}dppx)`), T.addEventListener("change", x), _());
  }
  const P = new ResizeObserver(([o]) => {
    if (l || !o) return;
    const r = o.contentRect;
    c === r.width && a === r.height || (c = r.width, a = r.height, _());
  }), M = new IntersectionObserver(([o]) => {
    l || !o || S === o.isIntersecting || (S = o.isIntersecting, _());
  });
  function k() {
    var o;
    l || (l = !0, cancelAnimationFrame(E), E = 0, P.disconnect(), M.disconnect(), T.removeEventListener("change", x), L.removeEventListener("change", _), document.removeEventListener("visibilitychange", _), window.removeEventListener("resize", x), (o = t.signal) == null || o.removeEventListener("abort", k), g());
  }
  return P.observe(e), M.observe(e), T.addEventListener("change", x), L.addEventListener("change", _), document.addEventListener("visibilitychange", _), window.addEventListener("resize", x), (w = t.signal) == null || w.addEventListener("abort", k, { once: !0 }), (H = t.signal) != null && H.aborted ? k() : b(), {
    setTheme(o) {
...
function C(t, n) {
  ...
  if (O(t, e, t.VERTEX_SHADER, N), O(t, e, t.FRAGMENT_SHADER, n), t.linkProgram(e), !t.getProgramParameter(e, t.LINK_STATUS)) throw new Error(`Shader failed to link: ${t.getProgramInfoLog(e)}`);
  return e;
}
function U(t, n, e) {
...
function G(t, n = {}) {
  ...
  return W(n, (f, d, m) => {
    ...
    const v = (b) => {
      e.bindFramebuffer(e.FRAMEBUFFER, c), e.useProgram(R), s(L, f, b), e.drawArrays(e.TRIANGLES, 0, 3), e.bindFramebuffer(e.FRAMEBUFFER, null), e.useProgram(T), s(A, f, b), e.uniform1f(A.uPixelRatio, m), e.activeTexture(e.TEXTURE0), e.bindTexture(e.TEXTURE_2D, a), e.uniform1i(A.tScene, 0), e.drawArrays(e.TRIANGLES, 0, 3);
    };
    if (d <= 0 || d >= 1) {
      v(d);
      return;
